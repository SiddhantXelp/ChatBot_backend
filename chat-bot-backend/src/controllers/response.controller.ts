...
const updateResponse = async (req: Request, res: Response) => {
  ...
  await ResponseModel.updateOne(
    { _id: id },
    { greeting, option, content, feedback }
  );
  const responseUpdated = await ResponseModel.findById(id, {
    greeting,
    option,
    content,
    feedback,
...
const deleteResponse = async (req: Request, res: Response) => {
  ...
  const id = req.params.id;
  try {
    const softdeleted = await ResponseModel.findByIdAndUpdate(
      { _id: id },
      {
        $set: {
          isDeleted: true,
...
const getRequestAndRelation = async (req: Request, res: Response) => {
  const { id } = req.params;
  try {
    const response: any = await ResponseModel.findOne({ uuid: id });

    const relations: any = await RelationModel.findOne({
    ...
    if (!relations) {
      res.send("Not family available");
    }
    var childResponses: Array<any> = [];

    const children = relations.children;

    for (let index = 0; index < children.length; index++) {
      try {
        let _childRes: any = await ResponseModel.find({
          uuid: children[index].uuid,
        });
        childResponses.push(..._childRes);
        //Send the response once we are at the end of the array
        if (index === children.length - 1) {
          let responses: any = { ...response._doc, children: childResponses };
          res.status(200).json({
